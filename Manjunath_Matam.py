...
Objective: Take two files (files.txt and nodes.txt) through the command line, process them, 
          distribute files to appropriate nodes, and produce an output file (result.txt). 

Distribution principle: The author has implemented the Min-Max distribution plan to produce a solution to balance the data stored on the nodes, not the space on the nodes. 
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        usage="Description: The program takes two input files (files.txt, nodes.txt) and produces an output file (result.txt) in three steps.",
        description="Description: The program takes two input files (files.txt, nodes.txt) and produces an output file (result.txt) in three steps.",
    )
    parser.add_argument(
        "-f",
        "-input-files",
    ...
    if len(fdf1) > 0:
        ...
        if len(ndf1) > 0:
            ...
            ### True positives
            tp = len(
                chk[(chk["Transferrable?"]) & ((chk["Node Space"] != np.nan))]
            )
            ### False positives
            fp = len(
                chk[(~chk["Transferrable?"]) & ((chk["Node Space"] == np.nan))]
